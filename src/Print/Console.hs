...
printWordsFromBySound langName sound =
  runSQLAction $ do
...
printLookupWordByAncestor text =
  runSQLAction $ do
...
printTranslate translationText =
  runSQLAction $ do
...
printNotEvolvedWordsFrom langName1 langName2 =
  runSQLAction $ do
...
printEvolveLaws langName1 langName2 =
  runSQLAction $ do
...
cReEvolveLangs langName1 langName2 =
  runSQLAction $ do
...
prettyEvolveResult (result, langFrom, langTo) =
  ...
  annotate
    ...
    ("from" <+>
     ...
     "to" <+> annotate bold (pretty (tshow langTo)))

tshowWord :: Word -> Text
tshowWord = tshowPretty prettyWord

tshowPretty :: (a -> Doc AnsiStyle) -> a -> Text
tshowPretty prettS value =
  renderStrict . layoutPretty defaultLayoutOptions $ prettS value

prettyWordDescription :: WordDescription -> Doc AnsiStyle
prettyWordDescription (word, langs, trans, wordAndLangsOrigins) =
  vsep
    [ ""
    , prettyEWord word <+> printLangList langs
    , if null trans
        then mempty
        else vsep ["", "       " <+> align (vsep (map prettyWT trans))]
    , if null wordAndLangsOrigins
        then mempty
        else annotate (color Black) $
             vsep
               [ ""
               , "       " <+>
                 align
                   (hsep
                      (punctuate semi (map prettyWordSource wordAndLangsOrigins)))
               ]
    , ""
    ]
  where
...
prettyShortTranslation (translation, _, mWord) =
  annotate
    (colorDull Blue)
    (case mWord of
...
prettyWordSource (wordAndLang, translations) =
  ...
  if null translations
    then mempty
    else (parens . hsep . punctuate comma . map prettyShortTranslation)
           translations

prettyWordAndLang :: WordAndLang -> Doc AnsiStyle
prettyWordAndLang (word, lang) =
  annotate
    (color Black)
    (annotate italicized "from" <+>
...
prettyWord word =
  ...
  annotate
    (color Black)
    (brackets $ annotate bold $ pretty (conShow $ wordPartOfSpeech word))
...
prettyEWord eWord =
  ...
  annotate
    (color Black)
    (brackets $
     annotate bold $ pretty (conShow $ (wordPartOfSpeech . entityVal) eWord)) <+>
  annotate
    (color Black)
    (parens $ (pretty . show . unSqlBackendKey . unWordKey . entityKey) eWord)
...
prettyWT (translation, toLang, mToWord) =
  ...
  annotate
    ...
    (annotate italicized "from" <+>
     ...
     if (null . translationComment) translation
       then mempty
       else (parens . pretty . translationComment) translation)
...
conShow Pronoun     = "pn."
conShow Verb        = "v."
conShow Prefix      = "prefix"
conShow Suffix      = "suffix"
