...
type WordDescriptionAPI
   = (WordJSON, [LanguageName], [WordTranslationAPI], [WordSourceAPI])
...
data AddWordTranslationJSON =
  AddWordTranslationJSON
    { translation      :: Maybe AddWordJSON
    , wordFromId       :: Int64
    , langTo           :: LanguageName
...
convertWordToWordJson (i, wrd) =
  WordJSON (Just i) (wordWord wrd) (wordPartOfSpeech wrd) (wordForgotten wrd)

convertToWordJson :: Word -> WordJSON
convertToWordJson wrd =
  WordJSON Nothing (wordWord wrd) (wordPartOfSpeech wrd) (wordForgotten wrd)

convertTranslationAPI :: Translation -> TranslationAPI
convertTranslationAPI tr =
  TranslationAPI
    ((fromSqlKey . DE.translationFromWordId) tr)
    ((fromSqlKey . DE.translationToLangId) tr)
    (map fromSqlKey (DE.translationToWordId tr))
    (DE.translationComment tr)
    (DE.translationAltTranslation tr)
...
convertWordTranslationAPI (tr, ln, mwrd) =
  (convertTranslationAPI tr, languageLname ln, map convertToWordJson mwrd)
...
convertWordSourceAPI (wl, listwt) =
  (convertWordAndLangAPI wl, map convertWordTranslationAPI listwt)
